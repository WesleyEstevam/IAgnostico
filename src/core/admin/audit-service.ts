...
export async function listAuditLogs(input: { category?: string; actor?: string; search?: string; from?: string; to?: string; page?: number }) {
  ...
  const category = ["user", "clinical_case", "clinicalCase", "plan", "staff", "siteContent", "support", "application_settings", "payment_settings", "coupon"].includes(input.category ?? "") ? input.category ?? "" : "";

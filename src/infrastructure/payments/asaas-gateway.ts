...
export class AsaasGateway implements PaymentGateway {
  ...
  private async request(path: string, init: RequestInit = {}): Promise<JsonObject> {
    ...
    if (!settings.enabled || !settings.apiKey) throw new Error("A integração Asaas ainda não está habilitada no painel administrativo.");
...
export async function getPaymentGateway(provider = "asaas"): Promise<PaymentGateway> {
  if (provider === "asaas") return new AsaasGateway();
  throw new Error("Provedor de pagamento não suportado.");
}

export async function syncAsaasWebhook(webhookUrl: string, email: string) {
  const settings = await getAsaasCredentials();
  if (!settings.enabled || !settings.apiKey || !settings.webhookToken) throw new Error("Salve e habilite as credenciais do Asaas antes de sincronizar o webhook.");
  const baseUrl = settings.environment === "production" ? "https://api.asaas.com/v3" : "https://api-sandbox.asaas.com/v3";
  ...
  const list = await call("/webhooks?limit=100");
  ...
  return typeof result.id === "string" ? result.id : typeof current?.id === "string" ? current.id : "sincronizado";
}
